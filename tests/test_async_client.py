...
async def _events(aiterable) -> list:
    return [e async for e in aiterable]
...
async def _run(base: str) -> None:
    async with AsyncPraixisClient(base, _API_KEY) as client:
        ...
        u = await client.chat.get_usage("abc")
        ...
        events = await _events(client.chat.stream("hi"))
        assert events[0] == {"type": "session_id", "value": "new-id"}, events
        assert _tokens(events) == "echo:hi", events

        # streaming file summary
        events = await _events(client.chat.summarize_file_stream(("report.txt", "hello doc")))
        ...
        events = await _events(client.rag.ask_stream("q?", collection_name="docs", session_id="s2"))
        assert {"type": "sources", "value": ["a.txt", "b.txt"]} in events, events
        assert _tokens(events) == "42", events

        hits = await client.rag.search("setup steps", collection_name="docs", n_results=3)
        ...
        assert _tokens(await _events(client.rag.compare_stream("docs", "v1.pdf", "v2.pdf"))) == "v1 vs v2"
        events = await _events(client.rag.summarize_document_stream("docs", "policy.pdf"))
